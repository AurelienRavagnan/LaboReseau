...
printf("Pour inet 192.168.242.128 : 1300  :\n");
if (getaddrinfo("192.168.242.128","1300",&hints,&results) != 0)
printf("Erreur de getaddrinfo");
else
{
...
{
struct addrinfo* addr;
  int result = getaddrinfo("Ravagnan", NULL, NULL, &addr);
  if (result != 0) {
    printf("Error from getaddrinfo: %s\n", gai_strerror(result));
  }
  else {
  struct sockaddr_in* internet_addr = (struct sockaddr_in*) addr->ai_addr;
  printf("Ravagnan is at: %s\n", inet_ntoa(internet_addr->sin_addr));
}
}
